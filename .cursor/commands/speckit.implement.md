...
1. **Initial commit**: From the repository root, commit all current changes before making any implementation changes. Use a message such as `chore: checkpoint before implement`. If the working tree is clean (nothing to commit), proceed. Then continue to step 2.
...
9. Progress tracking and error handling:
   ...
   - **After each completed task, commit the changes** with a clear message that describes the task (e.g. `feat: add X`, `test: add Y tests`). If there are no changes to commit for that task, skip the commit.
